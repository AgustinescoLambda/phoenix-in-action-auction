defmodule AuctionWeb.UserController do
  ...
  plug :prevent_unauthorized_acces when action in [:show]

  def index(conn,params) do
    redirect(conn, to: Routes.user_path(conn, :new, params))
  end
  def show(conn, %{"id" => id}) do
    user = Auction.get_user(id)
    bids = Auction.get_bids_from_user(user)
  ...
  defp prevent_unauthorized_acces(conn, _opts) do
    ...
    if current_user == nil || current_user.id != requested_user_id do
      conn
        |> put_flash(:error, "Buen intento, pero esto no es tuyo!")
